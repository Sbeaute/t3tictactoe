...
var oncreategame = function oncreategame(event) {
  ...
  api.creategame().then(ui.creategameSuccess).catch(ui.creategamesfailure);
};
...
var onupdategame = function onupdategame(event) {
  ...
  var squareVal = $(event.target).closest('a').text();
  console.log(squareVal);
  var squareId = $(event.target).closest('a').attr('id');
  ...
  api.updategames(squareVal, squareId).then(ui.updategameSuccess).catch(ui.updategameFailure);
};
module.exports = {
...
//   })
// }
var updategames = function updategames(squareVal, squareId, over) {
  return $.ajax({
...
module.exports = __webpack_require__.p + "1ffe86996db263c7e6bb5592516ac8ad.ttf";
